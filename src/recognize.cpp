...
{
    ros::init(argc, argv, "face_recognize_node");
    ros::NodeHandle node_recognize;
    ...
    CurDriverIDResult = new DriverIDResult();
    cout << "ArcFace 引擎序列化完成" << endl;

    CurAnalyzeStamp = ros::Time::now();
    StampInfoPub = node_recognize.advertise<driver_face::FaceIDMsg>("/camera_csi0/faceid_result", 1);

    image_transport::ImageTransport it(node_recognize);
